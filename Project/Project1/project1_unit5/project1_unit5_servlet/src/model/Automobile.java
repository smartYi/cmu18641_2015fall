...
public class Automobile implements Serializable {
	 ...
	 */
	public Automobile(String model, int size, float basePrice) {
		this.model = model;
	...
	public Automobile(String carmake, String model, float basePrice) {
		this.model = model;
	...
	public float getOptionPrice(String optionSetName, String optionName) {
		return findOptionset(optionSetName).getOption(optionName).getPrice();
	}
	 ...
	 */
	public void setOptionValue(int opsetIndex, String optionName, float price) {
		...
		for (int i = 0; i < optionSets.size(); i++) {
			if (optionSets.get(i) != null && optionSets.get(i).getName().equals(optionSetName)) {
				for (int j = 0; j < optionSets.get(i).getOpt().size(); j++) {
					if (optionSets.get(i).getOpt().get(j) != null
							&& optionSets.get(i).getOpt().get(j).getName().equals(optionName)) {
	...
	public void printOption() {
		...
		for (int i = 0; i < optionSets.size(); i++) {
			System.out.println(optionSets.get(i).getName() + ": " + getOptionChoice(optionSets.get(i).getName()) + ": "
