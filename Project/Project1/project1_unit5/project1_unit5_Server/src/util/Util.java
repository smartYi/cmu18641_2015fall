...
//This class is to read configurations from txt file and properties file to build auto objects
...
public class Util implements Serializable{
	...
	/**
	 * This method is to load a car model from properties file.
	 * @param properties
	 * @return
	 */
	public Automobile readAutoFromProperty(Properties properties) {
		
		Automobile automobile = null;
		
		//First build automobile model, car make and base price.
		automobile = new Automobile(properties.getProperty("CarMake"),
				                    properties.getProperty("CarModel"),
				                    Float.parseFloat(properties.getProperty("BasePrice")));
		
		//Then read its option set, options and option price.
		String optionSet = "Option";
		String optionValue = "OptionValue";
		String optionPrice = "OptionPrice";
		
		
		for(int optionSetIndex = 1; properties.getProperty(optionSet+optionSetIndex)!=null;optionSetIndex++) {
			
			//First read option set name and add it to the optionsets in an automobile
			automobile.setOptionsetValue(properties.getProperty(optionSet + optionSetIndex));
			
			//Then read several options under this option set.
			for(char optionIndex = 'a' ; properties.getProperty(optionValue + optionSetIndex + optionIndex) != null;
					optionIndex++) {
				String setName = properties.getProperty(optionSet + optionSetIndex);
				String option = properties.getProperty(optionValue + optionSetIndex + optionIndex);
				String price = properties.getProperty(optionPrice + optionSetIndex + optionIndex);
				
				automobile.setOptionValue(setName, option, Float.parseFloat(price));
			}
		}
		return automobile;
	}
	 ...
	 * @since 1.0
	 */
	public Automobile readFile(String filename) throws IOException,BlankFile,
	                                                    InvalidFileType,
	                                                    OptionNotFound,
	                                                    PriceNotFound{
				...
				for(int j=0;j<Integer.parseInt(opsetStr[1]);j++){
					...
					if(optStr.length == 1) {
						...
						if(val) {
							...
							al.setOptionValue(i,name,Float.parseFloat(optStr[0]));
						}
						...
						al.setOptionValue(i,optStr[0],Float.parseFloat(price));
						}
					}
					//If there is more than one data, it means no option price and no option name is missing.
					else {
						al.setOptionValue(i,optStr[0],Float.parseFloat(optStr[1]));
					}
				}
			}
